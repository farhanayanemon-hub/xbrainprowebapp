...
function injectEzboIdentity(messages: AIMessage[]): AIMessage[] {
	// Replace any existing leading system message(s) with our identity prompt prepended,
	// so the user can't leak the underlying model via a custom system prompt.
	const cleaned: AIMessage[] = [{ role: 'system', content: EZBO_IDENTITY_PROMPT }];
	for (const m of messages) {
		if (m.role === 'system') continue; // drop other system messages
		cleaned.push(m);
	}
	return cleaned;
}
